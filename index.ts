 ...
 */
export function strseg(string: string, segmentBy: Record<string, { from: string | RegExp, to: string | RegExp, reverse?: boolean } | string>): Record<string, string[]> {
    if (typeof segmentBy !== "object") {
        throw `strseg: segmentBy argument must be a record, such as {key: '<p>#>#</p>'`;
    }
    let recordReturn: Record<string, string[]> = {};
    ...
    for (let segmentKey of Object.keys(segmentBy)) {
        ...
        if (from instanceof RegExp) from = new RegExp(from.source, from.flags.replace('g', ''))
        if (to instanceof RegExp) to = new RegExp(to.source, to.flags.replace('g', ''))
        if (reverse) {
            [from, to] = [to, from];
